...
public readonly struct UnicodeCodePointRange : IEnumerable<int>
{
    ...
    private string RangeToString()
    {
        ...
        FirstCodePoint.TryFormat(buffer, out var length, "X4", CultureInfo.InvariantCulture);
        buffer.Slice(length, 2).Fill('.');
        length += 2;
        LastCodePoint.TryFormat(buffer[length..], out var l, "X4", CultureInfo.InvariantCulture);
        length += l;
    ...
    public static UnicodeCodePointRange Parse(string s)
    {
        ...
        var rangeSeparatorOffset = s.IndexOf("..", StringComparison.InvariantCulture);
